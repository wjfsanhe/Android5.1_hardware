...
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer)
{
	...
	if(debugFd == NULL)
		debugFd = fopen("/sdcard/debug.log","a+");
	//else
	//	fprintf(debugFd, "ztw %s %d",__func__,__LINE__);
...
//	ALOGE("ztw fps[%f]  %s %d", fps, __func__,__LINE__);
#else
                   ...
                   hw_device_t** device)
{
    ...
    if (!strcmp(name, GRALLOC_HARDWARE_FB0)) {
        ...
        if (status >= 0) {
            ...
            const_cast<float&>(dev->device.fps) =120; //m->fps;
