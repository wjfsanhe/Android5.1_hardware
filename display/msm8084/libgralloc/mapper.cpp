...
int gralloc_lock(gralloc_module_t const* module,
                 ...
                 void** vaddr)
{
    ALOGD("hw module gralloc lock");
    private_handle_t* hnd = (private_handle_t*)handle;
    ALOGD("version:%d numfds:%d",hnd->version,hnd->numFds);
    int err = gralloc_map_and_invalidate(module, handle, usage);
    if(!err){
        *vaddr = (void*)hnd->base;
	ALOGD("gralloc lock/map success *%p",hnd->base);
        ALOGD("Content:%x",*(char*)hnd->base);
    }
    return err;
}
